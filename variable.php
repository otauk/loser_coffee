                            ...
                            if ( is_array( $options ) ) {
                                ...
                                if ( taxonomy_exists( sanitize_title( $name ) ) ) {
                                    ...
                                    foreach ( $terms as $term ) {
                                        ...
                                        echo '<input type="radio" value="' . $term->slug . '" ' . checked( $selected_value, $term->slug, false ) . ' id="'. esc_attr( sanitize_title($name) ) .'_radio" name="attribute_'. sanitize_title($name).'_radio">' . apply_filters( 'woocommerce_variation_option_name', $term->name ).'<br />';
                                    }
                                } else {
                                    foreach ( $options as $option ) {
																				$select_options[] = '<option value="' . $option . '" ' . selected( $selected_value, $option, false ) . '>' . apply_filters( 'woocommerce_variation_option_name', $option ) . '</option>';
                                        echo '<input type="radio" value="' . $option . '" ' . checked( $selected_value, $option, false ) . ' id="'. esc_attr( sanitize_title($name) ) .'_radio" name="attribute_'. sanitize_title($name).'_radio">' . apply_filters( 'woocommerce_variation_option_name', $option ) . '<br />';
																		}
																}
                            }
                        ?>
